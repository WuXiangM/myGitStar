...
def copilot_summarize(repo):
    repo_name = repo["full_name"]
    desc = repo.get("description") or ""
    url = repo["html_url"]
    model = os.environ.get("GITHUB_COPILOT_MODEL", "gpt-4o-mini")
    ...
    try:
        resp = requests.post(
            "https://api.github.com/chat/completions",
            headers=headers,
            data=json.dumps(data),
            timeout=60
        )
        ...
        print(f"Copilot API 调用失败: {e}")
        return None


def summarize_batch(repos, old_summaries, use_copilot=False):
...
def copilot_summarize_batch(repos, old_summaries):
...
def main():
    ...
    for lang, repos in sorted(classified.items(), key=lambda x: -len(x[1])):
        ...
        for i in range(0, len(repos), 10):
            ...
            print(f"处理批次 {i//10 + 1}，包含 {len(this_batch)} 个仓库...")
