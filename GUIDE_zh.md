...
| `content_taxonomy_sample_strategy` | 否              | `head` / `random`                        | taxonomy 设计时如何抽样仓库                                                   | 推荐 `random` 更均衡；想保持旧行为就用 `head`                            |
| `content_taxonomy_sample_seed` | 否                  | `42`                                    | `random` 抽样的随机种子（固定后可复现）                                       | CI 建议固定一个值                                                     |
| `content_sort_categories_by_count` | 否               | `true` / `false`                        | 内容分类 README 中按类别数量降序排序，且 Other 放最后                          | 建议保持 `true`                                                        |
